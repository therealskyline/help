    ...
    try:
        ...
        if one_piece:
            ...
            if 'seasons' in one_piece and one_piece['seasons']:
                ...
                # Créer les sagas principales de One Piece
                regular_seasons = [
                    {"season_number": 1, "name": "Saga East Blue", "episodes": []},
                    {"season_number": 2, "name": "Saga Alabasta", "episodes": []},
                    {"season_number": 3, "name": "Saga Skypiea", "episodes": []}, 
                    {"season_number": 4, "name": "Saga Water Seven", "episodes": []},
                    {"season_number": 5, "name": "Saga Thriller Bark", "episodes": []},
                    {"season_number": 6, "name": "Saga Guerre au Sommet", "episodes": []},
                    {"season_number": 7, "name": "Saga Île des Hommes-Poissons", "episodes": []},
                    {"season_number": 8, "name": "Saga Dressrosa", "episodes": []},
                    {"season_number": 9, "name": "Saga Whole Cake Island", "episodes": []},
                    {"season_number": 10, "name": "Saga Pays des Wa", "episodes": []},
                    {"season_number": 11, "name": "Saga Egghead", "episodes": []}
                ]

                # Ajouter les saisons spéciales
                film_seasons = [{"season_number": 99, "name": "Films", "episodes": []}]
                kai_seasons = []

                # Parcourir les saisons existantes pour récupérer les épisodes
                for season in one_piece.get('seasons', []):
                    if season.get('season_number') == 99:
                        film_seasons[0]['episodes'].extend(season.get('episodes', []))
                    elif 'Kai' in season.get('name', ''):
                        kai_seasons.append(season)
                    else:
                        # Trouver la bonne saga pour ces épisodes
                        target_season = next((s for s in regular_seasons if s['season_number'] == season.get('season_number')), None)
                        if target_season:
                            target_season['episodes'].extend(season.get('episodes', []))
            ...
            try:
                ...
                if api_anime:
                    ...
                    if target_season:
                        ...
                        logger.error(f"Saison {season_num} non trouvée pour l'anime {anime['title']} - Vérifiez que la saison existe et que les épisodes sont disponibles")
            if anime['title'].lower() == 'one piece':
                logger.error("One Piece nécessite un rechargement - Utilisez la fonction preload_one_piece()")
                else:
